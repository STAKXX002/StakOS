...
static inline void do_sys_exit(int32_t code) { syscall3(SYS_EXIT, (uint32_t)code, 0, 0); }

static inline void do_sys_yield(void) { syscall3(SYS_YIELD, 0, 0, 0); }
...
static inline int do_sys_close(int fd) { return (int)syscall3(SYS_CLOSE, (uint32_t)fd, 0, 0); }
